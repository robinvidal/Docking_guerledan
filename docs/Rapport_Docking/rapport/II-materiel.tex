...
\begin{figure}[h!]
    ...
    \label{fig:bluerov_materiel}
\end{figure}

La figure \ref{fig:bluerov_materiel} montre notre BlueROV modifié pour le projet d'amarrage autonome.
...
Pour commencer, quelques tests ont été réalisés afin de valider le bon fonctionnement de notre algorithme de docking ; nous avons conçu une cage simple en PVC, visible sur la figure \ref{fig:cage} :

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{images/Cage.png}
    \caption{Image de notre cage en PVC}
    \label{fig:cage}
\end{figure}

Afin de la rendre plus visible par ce dernier, nous avons ajouté des mousses oranges sur les côtés verticaux de la cage.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        % Important : ici width=\textwidth signifie la largeur de la SOUS-FIGURE
        \includegraphics[width=\textwidth]{images/mousses_oranges.png}
        \caption{Image sonar avec mousses Oranges}
        \label{fig:mousse_a}
    \end{subfigure}
    \hfill % \hfill ajoute un espace élastique entre les images pour les écarter
    % --- Deuxième image ---
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/parbates.png}
        \caption{Image sonar avec parbates}
        \label{fig:mousse_b}
    \end{subfigure}
    \hfill
    % --- Troisième image ---
...
Le but de la cage est de permettre au ROV de s'amarrer. Il faut donc une cage complète (un cube), permettant au ROV d'entrer complètement à l'intérieur.
...
\begin{figure}[h!]
    ...
    \caption{Image de notre cage cubique en cours de conception}
    \label{fig:cage_cubique}
\end{figure}

Un grillage est en cours d'installation sur la face arrière et sur la face inférieure de la cage, afin de permettre au robot de rester correctement à l'intérieur.

Nous prévoyons également d'installer un entonnoir à l'entrée de la cage pour faciliter l'amarrage du ROV.
...
\subsection{Architecture Électronique Embarquée}

Rappelons brièvement l'architecture embarquée : le bas niveau est assuré par un Pixhawk, et le haut niveau (ROS2) tourne sur un ordinateur embarqué Raspberry Pi. L'interface entre ROS2 et le contrôleur de vol est assurée par le paquet \texttt{mavros}.
