...
La chaîne de traitement retenue suit la logique décrite dans \cite{wang2023_acoustic_mfls_docking} et adaptée aux images retournées par l’Oculus :

\begin{enumerate}
    \item \textbf{Pré-traitement :}  
    L’image sonar brute est d’abord convertie en matrice intensity–range. Un filtre médian réduit le bruit speckle, tandis qu’un seuillage adaptatif supprime le bruit de fond dû au plan d’eau ou aux réflexions parasites. Palomeras et al. \cite{palomeras2016_iauv_panel} soulignent que ce pré-traitement est indispensable pour éviter les fausses détections dans l’arrière-plan.

    \item \textbf{Extraction des caractéristiques :}  
    Les réflecteurs apparaissent comme deux zones d’intensité saturées. Une détection de ``blobs’’ (méthode basée sur les maxima locaux) est utilisée pour extraire leurs coordonnées polaires $(r, \theta)$.  
    Une alternative, utilisée dans certains travaux de la littérature \cite{yazdani2020_survey_docking}, serait d’exploiter une transformée de Hough polaire pour détecter les arêtes de la cage ; toutefois, dans la pratique, les points brillants des réflecteurs se révèlent beaucoup plus robustes et simples à exploiter.
\end{enumerate}
